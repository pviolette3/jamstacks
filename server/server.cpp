...
template <typename Request> class Handler {
public:
  Handler(RE2 pattern, RE2::Args *argv, int argc)
      : pattern_(pattern), argv_(argv), argc_(argc) {}
...
template <typename Request> class FacebookLoginHandler {
public:
...
private:
  using request_body_t =
      http::basic_dynamic_body<beast::flat_static_buffer<kBodySize>>;
  ...
  boost::asio::basic_waitable_timer<std::chrono::steady_clock>
      request_deadline_{acceptor_.get_io_service(),
  ...
  http::async_read(socket_, buffer_, *parser_, [this](beast::error_code ec) {
    if (ec)
      return accept();
